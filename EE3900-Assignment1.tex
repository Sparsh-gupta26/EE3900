...
\begin{equation}
    %\label{eq:idtft}
    H\brak{e^{\j \omega}} = \sum_{n=0}^{\infty}h(n) z^{-\j n \omega}
\end{equation}

Substitute the above acquired expression in \eqref{eq:idtft}
\begin{align}
    h(n) &= \frac{1}{2\pi} \int_{0}^{2\pi}  \sum_{k=0}^{\infty}h(k) z^{-\j k \omega}e^{\j n \omega}d \omega
    \\
    h(n) &= \frac{1}{2\pi}\sum_{k=0}^{\infty}h(k) \int_{0}^{2\pi} z^{\j (n-k) \omega} d \omega
\end{align}
The above integral is 1 when $k = n$ and $0$ otherwise.
\begin{equation*}
    \implies h(n) = h(n)
\end{equation*}

Hence we can say \eqref{eq:idtft} is a correct relation between $h(n)$ and $H(e^{\j \omega})$
